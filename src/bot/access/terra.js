...
module.exports = class Terraria {
    ...
    async monitor() {
        ...
        this.log = fs.createWriteStream(path.resolve(DATA_DIR, 'terra.log'), {
            flags: 'a',
        });
        ...
        const loop = async () => {
            const tcpLog = await dump(this.app.options.access.terra.port);
            const lines = tcpLog.split('\n');
            /** @type {Set<string>} */
            const ips = new Set();
            ...
            this.online = (
                await Promise.all(
                    [...ips].map(async ip => {
                        const uid = await TerraDB.get(ip);
    ...
    whitelist(ip, uid) {
        TerraDB.add(ip, uid);
        const sub = spawnSync(path.resolve(DATA_DIR, 'whitelist2.sh'), [ip], {
            timeout: 100,
        });
    ...
    handle(curr) {
        ...
        if (sub === 'access') {
            const TERRA = this.app.options.access.terra;
            ...
            this.pending.set(token, { type: 'terraria', user: curr.user });
            this.pendingUsers.set(curr.user.id, token);

            curr.reply({
                content: `**${TERRA.domain}:${TERRA.port}**`,
                components: [
                    R(
